...
pub struct SignalVar<'a, 'b, D> {
    value: D,
    change_receivers: Vec<Weak<Mutex<Box<dyn Receiver<Changed<'b, D>> + 'a>>>>,
}
...
impl<'a, 'b, D> Mutable<'a, 'b, D> for SignalVar<'a, 'b, D> where D: Clone + 'static {
        ...
        for receiver in &mut self.change_receivers {
            if let Some(receiver) = receiver.upgrade() {
                if let Ok(mut receiver) = receiver.lock() {
                    receiver.receive(&changed.clone());
                }
            }
        }
    }
    fn on_change<R>(&mut self, receiver: R) where R: Receiver<Changed<'b, D>> + Send + 'a{
